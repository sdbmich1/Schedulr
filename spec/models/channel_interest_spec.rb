...
describe ChannelInterest do
  ...
  before(:each) do
    ...
    @channel_interest = @channel.channel_interests.build(:interest_id => @interest.id)
  end
  ...
  describe "validations" do
    ...
    context "unique interests per channel" do
      ...
      it "should reject duplicate channel presenter" do
        @next_channel_interest = @channel.channel_interests.build(:interest_id => @interest.id)
	@next_channel_interest.should_not be_valid
      ...
      it "should accept unique channel interest" do
        ...
        @next_channel_interest = @new_channel.channel_interests.build(:interest_id => @interest.id)
	@next_channel_interest.should be_valid
      end

      it "should reject duplicate interests for the same channel" do
        @new_channel_interest = @channel.channel_interests.build(:interest_id => @interest.id)

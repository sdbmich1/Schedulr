...
describe ChannelLocation do
  ...
  before(:each) do
    ...
    @location = Factory.create :location, :country_id => 1
    @channel_location = @channel.channel_locations.build(:location_id => @location.id)
  end
  ...
  describe "validations" do
    ...
    context "unique locations per channel" do
      ...
      it "should reject duplicate channel presenter" do
        @next_channel_location = @channel.channel_locations.build(:location_id => @location.id)
	@next_channel_location.should_not be_valid
      ...
      it "should accept unique channel location" do
        ...
        @next_channel_location = @new_channel.channel_locations.build(:location_id => @location.id)
	@next_channel_location.should be_valid
      end

      it "should reject duplicate locations for the same channel" do
        @new_channel_location = @channel.channel_locations.build(:location_id => @location.id)

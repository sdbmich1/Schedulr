...
describe EventPresenter do
  before(:each) do
    ...
    @event_presenter = @event.event_presenters.build(:presenter_id => @presenter.id)
  end
  ...
  describe "validations" do
    ...
    context "unique presenters per event" do
      ...
      it "should reject duplicate event presenter" do
        @next_event_presenter = @event.event_presenters.build(:presenter_id => @presenter.id)
	@next_event_presenter.should_not be_valid
      ...
      it "should accept unique event presenter" do
        ...
        @next_event_presenter = @new_event.event_presenters.build(:presenter_id => @presenter.id)
	@next_event_presenter.should be_valid
      end

      it "should reject duplicate presenters for the same event" do
        @new_event_presenter = @event.event_presenters.build(:presenter_id => @presenter.id)
    ...
    context '.add_presenter_to_event' do
      
      before(:each) do
        @event = Factory(:event)
        @presenter = Factory(:presenter, :name => 'test lady')
        @event_presenter = @event.event_presenters.build(:presenter_id => @presenter.id)
        @event_presenter.save
      end

      it "should respond to add_presenter_to_event method" do
        @event_presenter.should respond_to(:add_presenter_to_event)
      end

    end

    context '.remove_presenter_from_event' do
      
      before(:each) do
        @event = Factory(:event)
        @presenter = Factory(:presenter, :name => 'test guy')
        @event_presenter = @event.event_presenters.build(:presenter_id => @presenter.id)
        @event_presenter.save
      end

      it "should respond to remove_presenter_from_event method" do
        @event_presenter.should respond_to(:remove_presenter_from_event)
      end
